...
# PART C — UI REQUIREMENTS DOCUMENT (UIRD)

# Sci‑Fi Real-Time User Interface Requirements for the Space Weather App

## 1. UI Vision

The Space Weather App interface must feel like a professional mission-control system rather than a normal weather dashboard. The user should feel that they are inside a live space operations console, watching the Sun, solar wind, magnetosphere, radiation environment, and Earth-response systems in real time.

The design must avoid static mock-style visuals. Every major visual block should be connected to real data, a live source timestamp, a refresh state, or a scientifically meaningful derived value. The interface should feel cinematic, but it must remain scientifically readable and operationally useful.

The UI should combine five design qualities:

1. **Mission-control seriousness** — reliable, disciplined, timestamped, source-aware.
2. **Sci-fi atmosphere** — dark interface, glowing data lines, orbit grids, plasma gradients, magnetic field visuals.
3. **Real-time confidence** — live badges, update intervals, data freshness indicators, stream animation.
4. **Scientific clarity** — units, axes, legends, uncertainty, source attribution, event context.
5. **Premium visual depth** — layered charts, 3D space environment, animated Earth, solar imagery, auroral overlays, and interactive timelines.

---

## 2. UI Design Theme

## 2.1 Visual Style Name

**Theme Name:** Helios Command Interface
...
5. Orbital grid.
6. Neon scientific instrumentation.
7. Live telemetry.
8. Space weather intelligence.
9. Observatory-grade dashboard.
10. Satellite operations console.

## 2.3 Color System

| Purpose | Suggested Color Direction |
|---|---|
| Background | Near-black, dark navy, deep cosmic blue |
| Primary Accent | Electric cyan / plasma blue |
| Solar Accent | Solar orange / flare yellow |
| Warning Accent | Red / magenta / amber |
| Geomagnetic Accent | Aurora green / violet |
| Radiation Accent | Purple / proton red |
| Neutral Text | Off-white / blue-gray |
| Secondary Text | Muted gray-blue |
| Grid Lines | Transparent cyan or gray-blue |
| Card Border | Subtle glowing outline |

## 2.4 Typography

The UI should use a combination of modern readable fonts and mission-console styled headings.

Recommended pairing:

1. **Primary UI Font:** Inter, IBM Plex Sans, or Manrope.
2. **Data/Telemetry Font:** JetBrains Mono, IBM Plex Mono, or Space Mono.
3. **Display/Section Header Font:** Rajdhani, Orbitron, Eurostile-style alternative, or Oxanium.

Typography rule:

1. Data values should use monospaced type.
2. Section headers should feel technical and futuristic.
3. Body text must remain easy to read.
4. Never sacrifice readability for sci-fi styling.

---

## 3. Main Experience Concept

The landing dashboard should behave like a live space weather operations wall.

The screen should be divided into:

1. **Central Space Environment View** — 3D Earth/Sun/magnetosphere visualization.
2. **Live Telemetry Strip** — Kp, Bz, solar wind speed, X-ray flux, proton flux, active alerts.
3. **Solar Activity Console** — GOES X-ray flux, flare classes, CME events.
4. **Radiation Console** — GOES proton flux, S-scale, particle thresholds.
5. **Geomagnetic Console** — planetary K index, G-scale, magnetosphere response.
6. **Impact Console** — satellite, GNSS, HF radio, aviation, power grid, aurora.
7. **Event Intelligence Timeline** — solar flare → CME → shock → geomagnetic storm chain.
8. **Alert Command Panel** — active alerts, warnings, watches, and recommended awareness notes.

---

## 4. Global UI Layout Requirements

## 4.1 Desktop Layout

The desktop layout should use a full-screen operational grid.

```text
┌───────────────────────────────────────────────────────────────┐
│ Top Mission Bar: Logo | Live Status | UTC Time | Source Health │
├───────────────┬───────────────────────────────┬───────────────┤
│ Left Nav      │ Central 3D Space Environment   │ Right Alerts  │
│ Modules       │ Earth + Sun + Magnetosphere    │ Impact Stack  │
├───────────────┴───────────────────────────────┴───────────────┤
│ Bottom Telemetry Dock: Kp | Bz | SW Speed | X-ray | Proton Flux│
└───────────────────────────────────────────────────────────────┘
```

## 4.2 Tablet Layout

The tablet layout should preserve the mission-console feel but stack secondary panels below the central view.

## 4.3 Mobile Layout

The mobile layout should become a vertical command feed:

1. Current condition.
2. Active alerts.
3. Kp and storm scale.
4. X-ray flux.
5. Solar wind.
6. Proton flux.
7. Impact cards.
...
Mobile must not hide scientific content. It should use collapsible panels and horizontal chart scrolling where needed.
...
This is the primary operational screen. It should answer: “What is happening in space weather right now?”

### Required UI Sections

1. Overall condition hero card.
2. 3D Earth and magnetosphere visual.
3. Live telemetry tiles.
4. GOES X-ray flux mini chart.
5. GOES proton flux mini chart.
6. Planetary K index mini chart.
7. Alert stack.
8. Sector-wise impact grid.
9. Event timeline strip.
10. Source health footer.

### Required Visual Feel

1. Animated star background with subtle parallax.
2. Rotating 3D Earth.
3. Sun direction indicator.
4. Magnetosphere field-line overlay.
5. Aurora oval glow when geomagnetic activity increases.
6. Data pulses when a feed updates.
7. Alert cards that glow softly based on severity.
8. Charts with gridline overlays resembling scientific instruments.

### Required Data Binding

Every item must be connected to one of the following:

1. NOAA SWPC data.
2. NASA DONKI event data.
3. NASA CCMC/iSWA future model product.
4. Derived rule-based impact score.
5. User-selected historical time range.

### Acceptance Criteria

1. No dashboard card may display fake values.
2. If real data is unavailable, the card must display “Waiting for live source” or “Source temporarily unavailable.”
3. Each card must display last updated timestamp.
4. Dashboard must update without full page reload.
5. The UI must visually distinguish live, stale, and unavailable data.

---

## 5.2 Solar Activity Screen

### Purpose

To show what the Sun is doing and whether solar activity may affect Earth.
...
1. **GOES X-ray Flux Chart**
   ...
   - Logarithmic y-axis.
   - A, B, C, M, X class labels.
   - 6-hour, 1-day, 3-day, and 7-day zoom buttons.
   - Peak flare annotations.
   - Hover tooltip with timestamp and flux value.

2. **Solar Flare Event Cards**
   - Flare class.
   - Begin, peak, and end time.
   - Active region number.
   - Source location.
   - Associated CME if available.

3. **Solar Disk View**
   - Real solar image layer if available.
   - Active region markers.
   - Flare source point.
   - Limb/disk location indicator.

4. **CME Intelligence Panel**
   - CME speed.
   - Angular width.
   - Source location.
   - Earth-directed flag.
   - Associated ENLIL model link if available.

5. **Solar Event Timeline**
   - Flare → CME → SEP → shock → geomagnetic response.
   - Causal links shown as connecting lines.

### Premium UI Behavior

1. X-class events should trigger a brief solar pulse animation.
2. M-class or higher events should appear in the top alert rail.
3. Earth-directed CME cards should show a trajectory-style arc toward Earth.
4. Event cards should expand into detailed science mode.

---

## 5.3 Solar Wind Screen

### Purpose

To show the live plasma stream traveling from the Sun toward Earth.

### Required Visualizations

1. **Solar Wind Speed Chart**
   - km/s line chart.
   - Threshold bands for normal, elevated, high-speed stream.
...
3. **Temperature Chart**
   - Kelvin or eV equivalent if available.

4. **IMF Bt and Bz Chart**
   - Bt total field.
   - Bz north/south component.
   - Southward Bz region highlighted.

5. **Plasma Stream Animation**
   - Animated flow from Sun to Earth.
   - Flow speed changes based on real solar wind speed.
   - Color intensity changes with density or magnetic field strength.
...
1. When Bz turns strongly southward, the magnetosphere visual should brighten or compress.
2. Solar wind speed should control the motion speed of particles in the background stream.
3. Data updates should animate smoothly without chart flicker.

---

## 5.4 Geomagnetic Activity Screen

### Purpose

To show how Earth’s magnetic field is responding to solar activity.

### Required Visualizations

1. **Planetary K Index Chart**
   - 3-hour bar chart.
   - Kp scale from 0 to 9.
   - NOAA G-scale color legend.
   - Current and forecast sections.

2. **Geomagnetic Storm Gauge**
   - G0 to G5 radial indicator.
   - Current Kp-driven level.

3. **Magnetosphere Compression View**
   - 3D Earth with field lines.
   - Field compression intensity based on Kp/solar wind/Bz rules.

4. **Aurora Oval Layer**
   - Aurora glow around polar regions.
   - Intensity based on Kp.
   - Optional hemispheric map view.

5. **Ground Impact Cards**
   - GNSS.
   - HF radio.
   - Power grid.
   - Aurora.
   - Satellite drag awareness.

### Premium UI Behavior

1. Kp 5 and above should trigger a storm-mode visual state.
2. Kp 7 and above should activate high-severity border animation.
3. Aurora oval should expand as Kp increases.
4. G-scale legend must always remain visible near the chart.

---

## 5.5 Radiation Environment Screen

### Purpose

To display energetic proton conditions and solar radiation storm awareness.

### Required Visualizations

1. **GOES Proton Flux Chart**
   - Multiple energy channels.
   - Logarithmic axis.
   - NOAA 10 MeV warning threshold line.
   - 6-hour, 1-day, 3-day, 7-day zoom.

2. **Radiation Storm Scale Gauge**
   - S0 to S5.
   - Current level.
   - Impact explanation.

3. **Particle Flux Stream View**
   - Particle field density increases with proton flux.
   - Radiation warning overlay when thresholds are exceeded.

4. **Satellite and Aviation Awareness Cards**
   - Satellite electronics risk.
   - Astronaut radiation awareness.
   - Polar aviation communication/radiation awareness.

### Premium UI Behavior

1. Threshold lines must be fixed and clearly labeled.
2. If proton flux crosses threshold, the UI should show a radiation alert strip.
3. Proton event cards should link to associated solar flare or CME if available.

---

## 5.6 Alerts and Warnings Screen

### Purpose

To show official alerts, watches, warnings, and internal threshold events.

### Required Visualizations

1. **Active Alert Stack**
   - Severity color.
   - Scale category: G, R, S, or general.
   - Issued time.
   - Validity period.
   - Source.
...
2. **Alert Timeline**
   - Alerts plotted by time and severity.

3. **Affected Systems Matrix**
   - Satellites.
   - GNSS.
   - Radio.
   - Aviation.
...
1. Alert cards should use cinematic but controlled severity glow.
2. Active alerts should remain pinned at top.
3. Expired alerts should fade but remain searchable.
4. Users should be able to switch between “Simple explanation” and “Technical detail.”

---

## 5.7 Event Timeline and Intelligence Screen

### Purpose

To help the user understand event chains and cause-effect relationships.

### Required Visualizations

1. **Horizontal Mission Timeline**
   - Solar flare.
   - CME.
   - SEP.
...
2. **Event Relationship Graph**
   - Node-link graph.
   - Events connected through DONKI linked events where available.
...
4. **Playback Mode**
   - Replay the last 24 hours, 3 days, or 7 days.
   - Animate charts and event cards over time.

### Premium UI Behavior

1. CME events should animate as outbound wavefronts.
2. Geomagnetic storm response should appear after related solar activity.
3. Timeline should allow zooming and panning.
4. User should be able to export selected event chain.

---

## 5.8 3D Space Environment Screen

### Purpose

To create the feeling of being inside a real-time heliophysics visualization room.

### Required Visualizations

1. **3D Earth**
   - Rotating Earth with day/night terminator.
   - Cloud texture optional.
   - Aurora glow overlay.

2. **Sun Direction Indicator**
   - Shows where the solar wind is coming from.
   - Direction arrow or glowing solar source.

3. **Magnetosphere Field Lines**
   - Stylized magnetic field lines.
   - Compression changes with storm conditions.

4. **Solar Wind Particle Flow**
   - Particle stream moving from Sun toward Earth.
   - Speed based on real solar wind speed.

5. **Satellite Orbit Layer**
   - Optional orbit rings for LEO, MEO, GEO.
   - Highlight GNSS orbit shell.

6. **Radiation Belt Layer**
   - Stylized Van Allen belt torus.
   - Intensity modulation during radiation storm conditions.
...
1. 3D scene must not block dashboard usability.
2. The 3D view must degrade gracefully on low-end devices.
3. Users must be able to disable heavy animations.
4. Scientific labels must be available as overlays.
5. The scene must use real values to drive animations wherever possible.

---

## 6. Visualization Catalogue

The app should include as many scientifically meaningful visuals as possible without overloading the user.

## 6.1 Real-Time Charts

| Visualization | Data | UI Purpose |
|---|---|---|
| GOES X-ray flux line chart | X-ray flux | Solar flare monitoring |
| GOES proton flux line chart | Proton flux | Radiation storm monitoring |
| Kp bar chart | Planetary K index | Geomagnetic activity |
| Solar wind speed line chart | Plasma speed | Solar wind monitoring |
| IMF Bz line chart | Magnetic field | Storm coupling awareness |
| Proton density chart | Plasma density | Shock/compression clues |
| Temperature chart | Solar wind temperature | Plasma state awareness |
| Bt/Bz combined chart | IMF values | Magnetic disturbance context |
| Alert timeline | Alerts | Warning history |
| Event timeline | DONKI events | Cause-effect tracking |

## 6.2 3D and Immersive Visuals

| Visualization | Description |
|---|---|
| Rotating Earth | Central real-time Earth scene |
| Magnetosphere field lines | Dynamic geomagnetic response visual |
| Solar wind particle stream | Animated plasma flow |
| Aurora oval | Kp-driven polar glow |
| Radiation belt torus | Radiation environment visual |
| CME wavefront | Event-driven solar eruption animation |
| Orbit shells | LEO, MEO, GEO, GNSS orbital context |
| Satellite icons | Optional spacecraft awareness layer |
| Sun-to-Earth line | Space weather propagation direction |
| Day/night terminator | Realistic Earth illumination context |

## 6.3 Map-Based Visuals

| Visualization | Description |
|---|---|
| Aurora visibility map | Kp-based aurora region estimate |
| GNSS impact map | Future TEC/scintillation layer |
| Ground station map | Optional institutional observatory/receiver points |
| Magnetic latitude overlay | Useful for aurora and storm interpretation |
| Global night map | Helps aurora viewing context |

## 6.4 Data Intelligence Visuals

| Visualization | Description |
|---|---|
| Event relationship graph | Links flare, CME, SEP, storm, alert |
| Impact matrix | Systems versus severity level |
| Source health matrix | Live status of NOAA, NASA, internal jobs |
| Threshold crossing log | When values crossed important limits |
| Forecast confidence panel | Future extension for model uncertainty |
| Data freshness heatmap | Which sources are live, stale, or delayed |

---

## 7. Real-Time UI Behavior Requirements

## 7.1 Live Update Rules

1. The UI shall update cards and charts using backend processed data.
2. The frontend shall not directly poll all external sources.
3. The UI shall use WebSocket or Server-Sent Events for high-frequency dashboard updates where appropriate.
4. The UI shall fall back to periodic API polling if live transport is unavailable.
5. Updates must animate smoothly without resetting zoom or filters.

## 7.2 Live Status Labels

Each panel must show one of the following:

1. **LIVE** — data updated within expected interval.
2. **DELAYED** — data is slightly older than expected.
3. **STALE** — data is old and may not represent current conditions.
4. **OFFLINE** — source unavailable.
5. **BACKFILLING** — historical data is being loaded.

## 7.3 Data Freshness Display

Each data card must display:

1. Source name.
2. Last received timestamp.
3. Last processed timestamp.
4. Refresh interval.
5. Current source health.

---

## 8. Motion and Animation Requirements

## 8.1 Allowed Animations

1. Subtle starfield motion.
2. Smooth chart line updates.
3. Data pulse when a new value arrives.
4. Soft glow for alerts.
5. Solar wind particle motion.
6. Rotating Earth.
7. Expanding CME wavefront.
8. Aurora intensity changes.
9. Magnetosphere compression animation.
10. Timeline playback animation.

## 8.2 Animation Restrictions

1. No excessive flashing.
2. No animation that reduces readability.
3. Critical alerts must be clear, not decorative only.
4. User must have reduced-motion option.
5. Heavy WebGL scenes must pause when browser tab is inactive.

---

## 9. UI Data Integrity Rules

1. Never display fake scientific values.
2. Never label simulated internal visuals as observational data.
3. Always separate measured data, derived data, and conceptual visualization.
4. Always show timestamp and source.
5. If an animation is derived from a rule, label it as “visual interpretation.”
6. If a value is unavailable, show unavailable state instead of zero.
7. Avoid misleading chart scaling.
8. Use logarithmic scale where scientifically required, especially for flux charts.

---

## 10. UI Accessibility Requirements

1. All critical information must be understandable without color alone.
2. Provide text labels for G, R, and S scales.
3. All charts must have table alternative view.
4. Keyboard navigation must work across panels.
5. Provide reduced-motion mode.
6. Provide high-contrast mode.
7. Font sizes must remain readable on mobile.
8. Tooltips must be accessible by keyboard focus.

---

# PART D — ADVANCED 2026 TECHNICAL REQUIREMENTS FOR A PREMIUM “$1M WEBSITE” FEEL

## 1. Technical Vision

The app should be engineered like a premium operational science platform. It should use modern frontend rendering, GPU-powered visualization, real-time transport, high-performance charting, progressive loading, source-aware backend ingestion, and a cinematic but scientifically grounded 3D layer.

The technical stack must support:

1. Real-time dashboard updates.
2. Large scientific time-series visualization.
3. 3D WebGL/WebGPU space environment rendering.
4. Animated charts and interactive event timelines.
5. Map-based aurora/GNSS layers.
6. Modular component architecture.
7. Production monitoring and graceful degradation.

---

## 2. Recommended Premium Frontend Stack

## 2.1 Core Framework

| Requirement | Recommended Library/Tool |
|---|---|
| Main application framework | Next.js or React |
| UI language | TypeScript |
| Component structure | React functional components |
| Server rendering / routing | Next.js App Router if using Next.js |
| Client-only visualization islands | Dynamic import / lazy loading |

### Rationale

React remains a strong base for component-based UI development. The app should use React or Next.js to organize mission panels, charts, 3D views, data stores, and route-level code splitting.

---

## 2.2 3D Space Environment

| Requirement | Recommended Library |
|---|---|
| WebGL 3D rendering | Three.js |
| React integration | React Three Fiber |
| 3D helper utilities | Drei |
| Animation | React Three Fiber frame loop + Framer Motion for UI |
| Post-processing | @react-three/postprocessing |
| Particle systems | Three.js Points, instanced meshes, or GPU particles |
| Globe rendering option | three-globe or custom Three.js sphere |

### Required 3D Features

1. Earth sphere with shader-based atmosphere.
2. Starfield background.
3. Aurora ring/oval shader.
4. Magnetic field line curves.
5. Solar wind particle stream.
6. CME wavefront animation.
7. Orbit shells and satellite markers.
8. Camera controls.
9. Performance mode toggle.

### Implementation Notes

1. Use dynamic import for the 3D scene so it does not block dashboard loading.
2. Use compressed textures where possible.
3. Use instancing for particle systems.
4. Use adaptive device pixel ratio.
5. Use reduced animation mode on weak devices.
6. Use Suspense loading for 3D assets.

---

## 2.3 Real-Time Scientific Charts

| Requirement | Recommended Library |
|---|---|
| High-performance charts | Apache ECharts |
| React wrapper | echarts-for-react or custom ECharts hook |
| Financial/technical style chart alternative | lightweight-charts |
| Scientific plotting alternative | Plotly.js |
| Simple dashboard charts | Recharts |

### Recommended Choice

Use **Apache ECharts** for the primary scientific charts because it supports many chart types, progressive rendering, dynamic data updates, interactive tooltips, zoom, timeline components, Canvas/SVG rendering choices, and large real-time datasets.
...
## 2.4 GPU-Powered Map and Layer Visualization

| Requirement | Recommended Library |
|---|---|
| High-performance geospatial layers | deck.gl |
| Map base | MapLibre GL or Mapbox GL JS |
| React map wrapper | react-map-gl |
| Lightweight map fallback | Leaflet |
| Scientific raster overlays | deck.gl BitmapLayer / TileLayer |
| Point and arc layers | deck.gl ScatterplotLayer / ArcLayer |

### Required Map Features

1. Aurora oval layer.
2. Night-side Earth layer.
3. Ground station markers.
4. Magnetic latitude overlay.
5. GNSS impact layer in future version.
6. TEC/scintillation raster layer in future version.
7. Satellite ground track layer.
8. Orbit-related arcs.

### Implementation Notes

1. Use deck.gl for GPU-powered large visual layers.
2. Use MapLibre GL if an open-source map rendering path is preferred.
3. Use Leaflet only for simple maps or low-end fallback.
4. Use tile/raster overlays for model products.

---

## 2.5 Animation and Premium UI Motion

| Requirement | Recommended Library |
|---|---|
| UI animations | Framer Motion / Motion for React |
| Number transitions | react-countup or custom spring animation |
| Canvas particles | tsParticles or custom Three.js particles |
| Page transitions | Framer Motion |
| Reduced motion support | CSS media query + app preference |

### Required Motion Features

1. Smooth card entrance.
2. Data value pulse on update.
3. Alert severity glow.
4. Timeline playback animation.
5. Panel expansion and collapse.
6. Floating telemetry dock.
7. Mission-control loading sequence.

---

## 2.6 State Management and Data Fetching
...
| Real-time stream state | WebSocket/SSE service + store |
| Form state | React Hook Form |
| Validation | Zod |

### Requirements

1. Use TanStack Query for API caching, retries, and background refresh.
2. Use Zustand for UI state such as selected time range, panel layout, theme, and animation mode.
3. Use Zod for validating API responses where critical.
4. Use optimistic UI only for user preferences, not scientific data.
...
| Component primitives | shadcn/ui or Radix UI |
| Complex enterprise components | Ant Design optional |
| Icons | Lucide React, Phosphor Icons |
| Fonts | Inter, JetBrains Mono, Rajdhani/Oxanium |
| CSS effects | CSS variables, gradients, backdrop-filter |

### Premium Styling Requirements

1. Glassmorphism only where readability remains strong.
2. Subtle glowing borders.
3. Card depth using layered shadows.
4. Mission grid background.
5. Animated but restrained alert states.
6. Consistent spacing scale.
7. Dark theme by default.
8. Optional light theme for reports and classroom use.

---

## 3. Recommended Backend Stack for Real-Time UI

| Requirement | Recommended Technology |
|---|---|
| Backend runtime | Node.js LTS |
| API framework | NestJS or Express.js |
| Language | TypeScript |
| Database | PostgreSQL + TimescaleDB or MongoDB time-series |
| Cache | Redis |
| Job queue | BullMQ |
...
## 3.1 Backend Recommendation

For a polished production-grade platform, use:

1. **NestJS + TypeScript** for structured backend modules.
2. **PostgreSQL + TimescaleDB** for serious time-series data.
3. **Redis + BullMQ** for ingestion queues and caching.
4. **SSE/WebSocket** for dashboard live updates.
5. **OpenAPI** for API documentation.

For a faster MERN-style institutional prototype, use:

1. React/Next.js.
2. Express.js.
3. MongoDB Atlas.
4. Redis cache.
5. Cron jobs or BullMQ workers.
...
## 4.2 Real-Time Transport Options

| Method | Use Case |
|---|---|
| REST polling | Simple panels and fallback mode |
| Server-Sent Events | One-way live dashboard updates |
| WebSocket | Interactive real-time subscriptions |
| GraphQL subscriptions | Optional advanced architecture |

### Recommended Approach

Use **Server-Sent Events** for dashboard live updates because space weather data mainly flows from server to client. Use WebSocket only if the app later requires collaborative sessions, live annotations, or bidirectional command channels.

---

## 5. Visualization-Specific Technical Requirements

## 5.1 GOES X-Ray Flux Chart
...
1. Use logarithmic y-axis.
2. Add horizontal class bands for A, B, C, M, X.
3. Add annotation markers for flare peak times.
4. Support zoom buttons: 6h, 1d, 3d, 7d.
5. Use smooth line rendering.
6. Use live update animation.
7. Preserve user zoom during refresh.
8. Provide data table fallback.

## 5.2 GOES Proton Flux Chart
...
1. Use logarithmic y-axis.
2. Display multiple proton energy channels.
3. Add NOAA threshold line.
4. Use color-coded threshold zones.
5. Mark S-scale transitions.
6. Support zoom buttons.
7. Provide event markers for SEP events.

## 5.3 Planetary K Index Chart
...
1. Use 3-hour bar chart.
2. Kp range from 0 to 9.
3. Add NOAA G-scale color zones.
4. Add current time marker.
5. Add forecast/observed separator if forecast data is included.
6. Add hover tooltip with universal time.
7. Add visual legend for Kp < 5, Kp = 5, 6, 7, 8, 9.

## 5.4 3D Magnetosphere Scene
...
1. Earth mesh.
2. Atmosphere shader.
3. Aurora shader or transparent mesh.
4. Magnetic field line curves.
5. Solar wind particles.
6. Dynamic compression factor.
7. CME wavefront mesh.
8. Camera controls.
9. Device performance detection.
10. Toggle labels.

## 5.5 Event Relationship Graph

### Library Options

1. React Flow for node-based event chains.
2. Cytoscape.js for graph networks.
3. D3-force for custom dynamic graph.

### Requirements

1. Nodes represent events.
2. Edges represent linked events.
3. Severity changes node glow.
4. Timeline position shown horizontally.
5. Clicking node opens detail drawer.
6. Export event chain as JSON/CSV.
...
1. Display polar map projection or global map.
2. Add aurora oval layer.
3. Add night-side mask.
4. Add Kp-based intensity.
5. Allow northern/southern hemisphere toggle.
6. Show disclaimer that actual visibility depends on weather, darkness, and location.

---

## 6. Performance Requirements for Premium UI

## 6.1 Frontend Performance

| Requirement | Target |
|---|---|
| Initial dashboard shell | Less than 2 seconds on broadband |
| First meaningful data display | Less than 3 seconds |
| Chart interaction latency | Less than 100 ms for common ranges |
| 3D scene frame rate | 30–60 FPS depending on device |
| Mobile fallback | No hard dependency on 3D scene |
| Route-level lazy loading | Required |
| Heavy visualization lazy loading | Required |

## 6.2 Optimization Techniques

1. Lazy load 3D scenes.
2. Lazy load ECharts modules by route.
3. Use memoized chart options.
4. Use data downsampling for long ranges.
5. Use server-side aggregation for historical charts.
6. Use Redis for latest values.
7. Use Web Workers for heavy frontend transformations.
8. Use OffscreenCanvas where appropriate.
9. Use CDN for textures and static assets.
10. Use progressive loading skeletons.

---

## 7. Premium Dashboard Component Inventory
...
## 7.3 Scientific Instrument Cards

Cards:

1. X-ray flux instrument.
2. Proton flux instrument.
3. Planetary K index instrument.
4. Solar wind plasma instrument.
5. IMF coupling instrument.
6. Event intelligence instrument.
7. Impact matrix instrument.

## 7.4 Immersive Scene Components

Components:

1. EarthScene.
2. SolarWindParticles.
3. MagnetosphereLines.
4. AuroraOval.
5. RadiationBelts.
6. OrbitShells.
7. CMEWavefront.
8. SpaceLabels.
9. CameraModeSelector.
10. PerformanceToggle.

---

## 8. File and Component Structure for Premium Frontend
...
client/src/
  ...
  components/
    ...
    space3d/
      ...
      AuroraOval.tsx
      MagnetosphereLines.tsx
      SolarWindParticles.tsx
      RadiationBelts.tsx
      OrbitShells.tsx
      CMEWavefront.tsx
    maps/
      AuroraMap.tsx
      GroundStationMap.tsx
      GNSSImpactMap.tsx
    timeline/
      EventTimeline.tsx
      EventNodeGraph.tsx
...
## 9. Backend Module Structure for Premium Build
...
## 10. Scientific UI Accuracy Requirements

1. Flux charts should use logarithmic scale.
2. Kp chart should use 0–9 scale.
3. NOAA G, R, and S scales should not be mixed incorrectly.
4. Bz southward should be visually distinct from northward Bz.
5. CME visuals must be labeled as interpretive unless model-based.
6. Aurora oval visual must be labeled as estimate/indicator unless official aurora model is used.
7. Data timestamps must be UTC by default.
8. User local time can be shown as secondary.
9. Source delay must be visible.
10. Derived impact labels must show the rule trigger.

---

## 11. “No Mock Data” Development Policy

The project must follow a strict no-mock-data display policy for production-like screens.

### Allowed During Development

1. Local seed data for offline component testing.
2. Storybook visual fixtures clearly labeled as fixtures.
3. Unit test fixtures.
4. Placeholder skeletons without scientific values.

### Not Allowed in Production UI

1. Fake Kp values.
2. Fake solar wind values.
3. Fake flare classes.
4. Fake alerts.
5. Static screenshots pretending to be live data.
6. Visual effects that imply real events without real data trigger.

### Production Rule

If data is missing, the UI must show:

1. Source unavailable.
2. Waiting for live feed.
3. Last known value with stale warning.
4. Historical mode label.

---

## 12. Recommended Library Stack Summary

## 12.1 Premium Build Stack
...
| Scientific charts | Apache ECharts |
| Maps/layers | deck.gl + MapLibre GL |
...
| Real-time | Server-Sent Events + optional WebSocket |
| Monitoring | Sentry + Grafana + Prometheus |
| API docs | OpenAPI/Swagger |

## 12.2 Fast MERN Prototype Stack

| Layer | Recommended Choice |
|---|---|
| Frontend | React + Vite |
| UI | Ant Design or Tailwind CSS |
| Charts | Apache ECharts/Recharts |
| 3D | React Three Fiber |
| Backend | Express.js |
| Database | MongoDB Atlas |
| Cache | Redis optional |
| Jobs | node-cron or BullMQ |
| Real-time | Socket.IO or SSE |

---

## 13. UI Quality Acceptance Criteria

The UI will be considered successful if:

1. It feels like a real mission-control dashboard within the first 5 seconds.
2. It does not use fake scientific numbers.
3. It shows live status, source status, and timestamps everywhere.
4. It includes at least three live scientific charts.
5. It includes at least one immersive 3D Earth/magnetosphere scene.
6. It includes NOAA-scale legends for G, R, and S events.
7. It includes event timeline intelligence.
8. It includes sector-wise impact cards.
9. It remains usable on laptop screens.
10. It has a reduced-motion/performance mode.
11. It explains technical values in simple language when clicked.
12. It looks premium without compromising scientific honesty.

---

## 14. Final UI Deliverable Definition

The final UI should deliver the feeling of a live space weather command center with the following minimum visual package:

1. Full-screen dark sci-fi dashboard.
2. Live telemetry header.
3. GOES X-ray flux chart like an operational instrument.
4. GOES proton flux chart with warning threshold.
5. Planetary K index chart with NOAA storm scale legend.
6. 3D Earth with aurora and magnetosphere field lines.
7. Solar wind particle stream animation driven by real speed values.
8. Alert command panel.
9. Event timeline and relationship graph.
10. Impact matrix for satellites, GNSS, aviation, radio, power, and aurora.
11. Source health and data freshness layer.
12. Responsive mobile command-feed mode.
13. Accessibility and reduced-motion support.
14. Exportable data and chart views.
15. Clear distinction between real observations, forecasts, and visual interpretations.

---

## 15. Development Priority for Premium Visual Feel

The best order for implementation is:

1. Build real data ingestion first.
2. Build dashboard shell and live status system.
3. Build X-ray, proton, and Kp charts.
4. Build telemetry tiles.
5. Build alerts and source health.
6. Build impact scoring cards.
7. Add 3D Earth scene.
8. Add magnetosphere and aurora effects.
9. Add event timeline and relationship graph.
10. Add advanced map layers.
11. Add animation polish.
12. Add performance and accessibility controls.

This order ensures the app remains real and scientific before becoming cinematic.
